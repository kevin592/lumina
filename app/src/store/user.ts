...
export class UserStore implements Store {
  ...
  tokenData = new StorageState<TokenData | null>({ key: 'LuminaToken', value: null });
  theme: any = 'light';
  ...
  canRegister = new PromiseState({
    function: async () => {
      if (isInTauri() && getluminaEndpoint() == '') {
        return
      }
  ...
  async initializeSettings(setTheme: (theme: string) => void, i18n: any) {
    ...
    const config = await this.Lumina.config.call()
    const handleFeatureRoute = (
      ...
      const configKey = featureKey === 'ai' ? 'mainModelId' : `isUseLuminaHub`;
