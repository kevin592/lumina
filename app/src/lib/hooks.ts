...
import { ShowEditLuminaModel } from '@/components/LuminaRightClickMenu';
...
export const useConfigSetting = (configKey: keyof LuminaStore['config']['value']) => {
  ...
  useEffect(() => {
    if (Lumina.config.value && Lumina.config.value[configKey]) {
      store.setValue(Lumina.config.value[configKey] as string);
    }
  }, [Lumina.config.value, configKey]);
...
const initializeAndroidShortcuts = () => {
  ...
  const checkAndroidData = () => {
      ...
      if (action) {
        ...
        switch (action) {
          case 'quick_note':
            ShowEditLuminaModel('2xl', 'create');
      ...
      if (shareDataStr && !isProcessingSharedData) {
        ...
        try {
          ...
          if (shareData.text) {
            ...
            ShowEditLuminaModel('2xl', 'create', { text: cleanText });
            ...
            readFile(shareData.stream).then(contents => {
              ...
              console.log('xxx!!!')
              ShowEditLuminaModel('2xl', 'create', { file });
              isProcessingSharedData = false;
            }).catch((error: Error) => {
