  ...
  useEffect(() => {
    ...
    const fetchImage = async () => {
      ...
      try {
        ...
        const response = await axiosInstance.get(getluminaEndpoint(`${src}?thumbnail=true`), {
          responseType: 'blob'
        });
    ...
    <>
      {loading && (
        <div className="flex items-center justify-center w-full h-full">
          <i className="ri-loader-4-line animate-spin" style={{fontSize: "24px"}}></i>
        </div>
      )}
      {!loading && (
        <Image
    ...
    <div className={`relative group ${!preview ? 'min-w-[160px] flex-shrink-0' : ''} ${imageHeight}`}>
      {file.uploadPromise?.loading?.value && (
        <div className='absolute inset-0 flex items-center justify-center w-full h-full'>
          <i className="ri-upload-cloud-line animate-spin" style={{fontSize: "40px"}}></i>
        </div>
      )}
      <div className='w-full'>
        <PhotoView src={getluminaEndpoint(`${file.preview}?token=${RootStore.Get(UserStore).tokenData.value?.token}`)}>
          <div>
