...
export const LuminaSelectNote = observer(({ iconButton, onSelect, blackList = [], tooltip = 'reference', autoClose = true }: Props) => {
  const Lumina = RootStore.Get(LuminaStore);
  const [isOpen, setIsOpen] = useState(false);
  const { t } = useTranslation();

  const defaultIconButton = <IconButton tooltip={t(tooltip)} icon="ri-link" />;
    ...
    >
      ...
      <PopoverContent className="flex flex-col max-w-[300px]">
        ...
        <ScrollArea
          ...
          onBottom={() => {
            Lumina.referenceSearchList.callNextPage({});
          }}
        >
          {Lumina.referenceSearchList?.value?.map((item) => (
            <div
              key={item.id}
