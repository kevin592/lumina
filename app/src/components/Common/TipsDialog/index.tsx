    ...
    <PopoverContent>
      <div className="px-1 py-2 flex flex-col">
        ...
        <div className='flex my-1 gap-2'>
          <Button startContent={<i className="ri-close-line" style={{fontSize: "20px"}}></i>} variant="flat" size="sm" className="ml-auto" color='default' onPress={e => {
            RootStore.Get(DialogStandaloneStore).close()
            props.onCancel?.()
          }}>{t('cancel')}</Button>
          <Button startContent={<i className="ri-check-line" style={{fontSize: "20px"}}></i>} isLoading={isLoading}  size="sm" color='danger' onPress={async e => {
            props.onConfirm?.()
          }}>{t('confirm')}</Button>
        </div>
      </div>
