...
export const ImportProgress = observer(({ force }: { force: boolean }) => {
  ...
  const getStatusIcon = (type: string) => {
    switch (type) {
      case 'success':
        return <i className="ri-checkbox-circle-fill text-green-500" style={{fontSize: "16px"}}></i>;
      case 'error':
        return <i className="ri-close-circle-fill text-red-500" style={{fontSize: "16px"}}></i>;
      case 'skip':
        return <i className="ri-refresh-line text-yellow-500" style={{fontSize: "16px"}}></i>;
      case 'info':
        return <i className="ri-information-fill text-blue-500" style={{fontSize: "16px"}}></i>;
      default:
        return <i className="ri-checkbox-blank-circle-fill text-gray-400" style={{fontSize: "16px"}}></i>;
    }
  };
  ...
  return <div className="space-y-4">
    <div className="space-y-4">
      ...
      <div className="flex justify-end items-center">{/* 移除�?justify-between，只保留右对�?*/}
        ...
        {store.status === 'running' && (
          <button
            ...
            title={t('stop-task')}
          >
            <i className="ri-stop-circle-fill" style={{fontSize: "16px"}}></i>
            {t('stop-task')}
          </button>
        )}
        ...
        {store.isSuccess && (
          <div className="flex items-center gap-2 px-4 py-2 bg-green-50 dark:bg-green-950/50 text-green-600 dark:text-green-400 rounded-full text-sm font-medium">
            <i className="ri-checkbox-circle-fill" style={{fontSize: "16px"}}></i>
            {t('completed')}
          </div>
        )}
        ...
        {store.isError && (
          <div className="flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-950/50 text-red-600 dark:text-red-400 rounded-full text-sm font-medium">
            <i className="ri-close-circle-fill" style={{fontSize: "16px"}}></i>
            {t('error')}
          </div>
        )}
      </div>
    ...
    <div className='flex flex-col max-h-[400px] overflow-y-auto mt-4 space-y-2 pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent'>
      ...
      {store.message.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-gray-400">
          <i className="ri-loader-4-line animate-spin mb-2" style={{fontSize: "32px"}}></i>
          <div>{t('loading')}...</div>
        </div>
      )}
    </div>
