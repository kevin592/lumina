...
export const MobileNavBar = observer(({ onItemClick }: MobileNavBarProps) => {
  ...
  if (lumina.config.value?.isHiddenMobileBar) {
    return null;
  }
  ...
  return (
    <motion.div
      className="Lumina-bottom-bar h-[60px] flex w-full px-3 py-2 gap-1 bg-background block md:hidden overflow-hidden fixed bottom-0 z-50"
