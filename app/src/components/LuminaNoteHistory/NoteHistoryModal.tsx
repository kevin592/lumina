  ...
  const Store = RootStore.Local(() => ({
    ...
    handleRestore: async () => {
      ...
      try {
        ...
        RootStore.Get(LuminaStore).updateTicker++;
      ...
      {Store.historyList.value && Store.historyList.value.length > 0 && (
        <>
          <div className="flex flex-col items-center mb-4 px-4">
            <div className="w-full flex justify-between items-center mb-2">
              <span className="text-sm flex items-center text-desc">
                <Icon icon="ri:time-line" className="mr-1 text-desc" />
                {t('newer')}
              </span>
              <span className="text-sm font-medium text-primary">{Store.getVersionLabel()}</span>
              <span className="text-sm flex items-center text-desc">
                {t('older')}
                <Icon icon="ri:time-fill" className="ml-1 text-desc" />
              </span>
            </div>
            ...
            <div className="w-full mt-2">
              <p className="text-xs text-amber-500 flex items-center">
                <Icon icon="ri:information-line" className="mr-1" width={14} height={14} />
                {t('history-note-only')}
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end mt-4">
            <Button color="primary" isLoading={Store.restoring} onPress={Store.handleRestore} startContent={<Icon icon="ri:restore-line" />}>
