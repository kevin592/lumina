...
import { LuminaCard, LuminaItem } from "../LuminaCard";
                ...
                <Popover placement="bottom-start">
                  <PopoverTrigger>
                    <Button
                      ...
                      startContent={<i className="ri-file-line text-xs"></i>}
                    >
                    ...
                    onClick={async () => {
                      ...
                      if (aiStore.currentConversation.value) {
                        try {
                          ...
                          const shareData = `Lumina-ai-share-${conversationId}`;
                          const encodedId = btoa(shareData);
                          const shareUrl = getluminaEndpoint(`/ai-share/${encodedId}`)
                          copy(shareUrl);
...
export const LuminaChatBox = observer(({ shareMode = false }: { shareMode?: boolean } = {}) => {
  const aiStore = RootStore.Get(AiStore)
        ...
        <AnimatePresence>
...

          {
            aiStore.isAnswering && !aiStore.currentMessageResult.content && (
              <div className="text-desc flex gap-1">
