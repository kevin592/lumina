...
export const ResourceItemPreview = ({
  ...
  return (
    <div className={`w-full flex items-center gap-2 p-2 rounded-md cursor-pointer group ${className}`} onClick={onClick}>
      {isImage ? (
        <PhotoProvider>
          <PhotoView src={getluminaEndpoint(`${item.path}?token=${RootStore.Get(UserStore).tokenData.value?.token}`)}>
            <div>
        ...
        <div className="font-medium text-sm flex items-center gap-2">
          ...
          {isS3File && showExtraInfo && (
            <Tooltip content={t('cloud-file')}>
              <Icon icon="ri:cloud-2-fill" className="w-4 h-4" />
            </Tooltip>
          )}
          {showAssociationIcon && !item.noteId && showExtraInfo && (
            <Tooltip content={t('no-note-associated')}>
              <Icon icon="ri:alert-line" className="w-4 h-4 text-yellow-500" />
            </Tooltip>
          )}
        </div>
        {showExtraInfo && (
...
// Design v2.0 - Linear 风格列表样式
const getCardClassName = (isDragging?: boolean, isDraggingOver?: boolean) => {
  const baseClasses = 'mb-2 px-4 py-3.5 hover:bg-gray-50 bg-white !transition-all duration-200 rounded-xl border border-transparent hover:border-gray-100';

  if (isDraggingOver) {
    return `${baseClasses} relative overflow-visible ring-2 ring-blue-400/30 bg-blue-50/50`;
  }

  if (isDragging) {
    return `${baseClasses} opacity-50 bg-blue-50/50`;
  }
      ...
      <Card {...cardProps} shadow="none">
        <div className="flex items-center gap-4 ml-[45px]">
          <div className="w-[36px] h-[36px] ml-[-7px] flex items-center justify-center">
            <Icon icon="ri:folder-fill" className="w-full h-full text-yellow-500" />
          </div>
