    ...
    <Dropdown onOpenChange={e => onTrigger()}>
      <DropdownTrigger>
        <Button
          ...
          variant="light"
        >
          <Icon icon="ri:more-2-fill" width="20" height="20" />
        </Button>
      </DropdownTrigger>
      <DropdownMenu aria-label="Resource Actions">
        {
          resource?.isFolder ? null : (
            <DropdownItem key="download" onPress={handleDownload}>
              <MenuItem icon="ri:download-line" label={t('download')} />
            </DropdownItem>
          )
        }

        <DropdownItem key="rename" onPress={handleRename}>
          <MenuItem icon="ri:edit-line" label={t('rename')} />
        </DropdownItem>
...

        {
          resource?.isFolder ? null : (
            <DropdownItem key="cut" onPress={handleCut}>
              <MenuItem icon="ri:scissors-cut-line" label={t('cut')} />
            </DropdownItem>
          )
        }
        ...
        {canPaste() ? (
          <DropdownItem key="paste" onPress={handlePaste}>
            <MenuItem icon="ri:clipboard-line" label={t('paste')} />
          </DropdownItem>
        ) : null}
