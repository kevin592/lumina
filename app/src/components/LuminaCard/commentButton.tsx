...
export const UserAvatar = observer(({ account, guestName, isAuthor, LuminaItem, withoutName, size = 20 }: {
  ...
  return (
    <div className="flex items-center gap-2">
      {account ? (
        <>
          {account.image ? (
            <FallbackImage src={LuminaItem?.originURL ? (LuminaItem.originURL + account.image) : getluminaEndpoint(account.image + `?token=${RootStore.Get(UserStore).tokenData.value?.token}`)} radius="full" alt="" width={size} height={size} />
          ) : (
            <Avatar
              size={size}
              name={displayName}
          ...
          {isAuthor && LuminaItem && String(account.id) === String(LuminaItem.accountId) && (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <i className="ri-chat-3-line" style={{ fontSize: '16px' }} />
          </Button>
          {(user.id === comment.note?.account?.id || user.id === comment.account?.id) && !LuminaItem.originURL && (
            <Button
              size="sm"
              variant="light"
              color="danger"
...
export const CommentDialog = observer(({ LuminaItem }: { LuminaItem: LuminaItem }) => {
  const { t } = useTranslation();
  ...
  const Store = RootStore.Local(() => ({
    ...
    commentList: new PromisePageState({
      function: async ({ page, size }) => {
        if (LuminaItem.originURL) {
          const res = await axios.post(LuminaItem.originURL + '/api/v1/comment/list', {
    ...
    handleSendComment: new PromiseState({
      function: async ({ content }: { content: string }) => {
        ...
        if (LuminaItem.originURL) {
          await axios.post(LuminaItem.originURL + '/api/v1/comment/create', {
            ...params,
    ...
    handleDelete: new PromiseState({
      function: async (commentId: number) => {
        if (LuminaItem.originURL) {
          await axios.post(LuminaItem.originURL + '/api/v1/comment/delete', {
            id: commentId
          });
        } else {
      ...
      <AnimatePresence>
        {Store.reply.id && (
          ...
          >
            ...
            <i
              className="ri-close-line cursor-pointer text-default-400 hover:text-default-500"
              style={{ fontSize: '18px' }}
...
export const SimpleCommentList = observer(({ LuminaItem }: { LuminaItem: LuminaItem }) => {
  const { t } = useTranslation();
  const commentList = LuminaItem.comments;
...
export const ShowCommentDialog = async (noteId: number) => {
  ...
  try {
    ...
    const noteDetail = await Lumina.noteDetail.call({ id: noteId });
    ...
    dialog.setData({
      ...
      content: <CommentDialog LuminaItem={noteDetail} />
    });
...
export const CommentButton = observer(({ LuminaItem, alwaysShow = false }: { LuminaItem: Note, alwaysShow?: boolean }) => {
  const { t } = useTranslation();
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    RootStore.Get(DialogStore).setData({
      isOpen: true,
      size: 'lg',
      title: `${i18n.t('comment')} ${LuminaItem._count?.comments ? `(${LuminaItem._count.comments})` : ''}`,
      content: <CommentDialog LuminaItem={LuminaItem} />
    });
  };
  ...
  return (
    <Tooltip content={t('comment')} delay={1500}>
      <div className="flex items-center gap-2">
        <i
          className={`ri-chat-3-line cursor-pointer ml-2 ${isIOSDevice
            ? 'opacity-60'
...
export const CommentCount = observer(({ LuminaItem }: { LuminaItem: Note }) => {
  if (LuminaItem?._count?.comments == 0) return null;
  const handleClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    RootStore.Get(DialogStore).setData({
      isOpen: true,
  ...
  return (
    <div className="flex items-center gap-1 hover:bg-background rounded-full px-1 py-0.5 cursor-pointer" onClick={handleClick}>
      <CommentButton LuminaItem={LuminaItem} alwaysShow={true} />
      <span className="text-sm text-ignore">{LuminaItem?._count?.comments}</span>
    </div>
  );
});
