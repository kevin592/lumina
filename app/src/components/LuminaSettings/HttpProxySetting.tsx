...
export const HttpProxySetting = observer(() => {
  ...
  useEffect(() => {
    store.isUseHttpProxy = Lumina.config.value?.isUseHttpProxy!;
    store.httpProxyHost = Lumina.config.value?.httpProxyHost!;
    store.httpProxyPort = Lumina.config.value?.httpProxyPort! || 0;
    store.httpProxyUsername = Lumina.config.value?.httpProxyUsername!;
    store.httpProxyPassword = Lumina.config.value?.httpProxyPassword!;
  }, [Lumina.config.value]);
                ...
                endContent={
                  ...
                  >
                    {store.isProxyPasswordVisible ? (
                      <Icon icon="ri:eye-close-line" className="text-2xl pointer-events-none" />
                    ) : (
                      <Icon icon="ri:eye-line" className="text-2xl pointer-events-none" />
                    )}
                  </button>
                }
              />
                ...
                <div className="flex flex-col gap-3 mt-2 w-full">
                  ...
                  {Object.keys(testResult).length > 0 && (
                    <div className={`p-3 rounded-lg mt-2 ${testResult.success ? 'bg-success-50 text-success-600' : 'bg-danger-50 text-danger-600'}`}>
                      <div className="flex items-center gap-2">
                        {testResult.success 
                          ? <Icon icon="ri:checkbox-circle-line" width="20" height="20" /> 
                          : <Icon icon="ri:error-warning-line" width="20" height="20" />
                        }
