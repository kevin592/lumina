...
export const StorageSetting = observer(() => {
  ...
  useEffect(() => {
    store.s3AccessKeyId = Lumina.config.value?.s3AccessKeyId!
    store.s3AccessKeySecret = Lumina.config.value?.s3AccessKeySecret!
    store.s3Endpoint = Lumina.config.value?.s3Endpoint!
    store.s3Region = Lumina.config.value?.s3Region!
    store.s3Bucket = Lumina.config.value?.s3Bucket!
    store.s3CustomPath = Lumina.config.value?.s3CustomPath!
    store.localCustomPath = Lumina.config.value?.localCustomPath!
  }, [Lumina.config.value])
  ...
  >
    ...
    {Lumina.config.value?.objectStorage != 's3' &&
      <Item
        leftContent={<>
