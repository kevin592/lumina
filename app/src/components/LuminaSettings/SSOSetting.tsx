...
const UpdateSSOProvider = observer(({ provider }: { provider?: z.infer<typeof ZOAuth2ProviderSchema> }) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    upsertProvider: new PromiseState({
      function: async () => {
        const config = Lumina.config.value || {}
...
export const SSOSetting = observer(() => {
  const { t } = useTranslation()
  const Lumina = RootStore.Get(LuminaStore)
  const providers = Lumina.config.value?.oauth2Providers || []
                  ...
                  <TableCell>
                    <div className="flex">
                      <Button
                        ...
                        variant="flat"
                        size="sm"
                        startContent={<Icon icon="ri:edit-line" width="18" height="18" />}
                        onPress={() => {
                          RootStore.Get(DialogStore).setData({
                            isOpen: true,
                      ...
                      <Button
                        ...
                        size="sm"
                        className="ml-2"
                        startContent={<Icon icon="ri:delete-bin-line" width="18" height="18" />}
                        onPress={() => {
                          showTipsDialog({
                            size: 'sm',
