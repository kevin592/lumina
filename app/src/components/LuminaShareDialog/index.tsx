...
export const LuminaShareDialog = observer(({ defaultSettings }: ShareDialogProps) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    async handleCreateShare() {
      ...
      if (this.selectedTab === "public") {
        const res = await RootStore.Get(LuminaStore).shareNote.call({
          id: RootStore.Get(LuminaStore).curSelectedNote!.id!,
          isCancel: false,
          password: this.isPublic ? "" : this.settings.password,
          expireAt: this.settings.expiryDate
        });
        const luminaEndpoint = getluminaEndpoint() ?? window.location.origin;
        this.setShareUrl(luminaEndpoint + 'share/' + (res?.shareEncryptedUrl ?? '') + (this.isPublic ? '' : '?password=' + (this.settings.password ?? '')));
        this.setIsShare(true);
      }   
      // Handle internal sharing
      else if (this.selectedTab === "internal") {
        await RootStore.Get(LuminaStore).internalShareNote.call({
          id: RootStore.Get(LuminaStore).curSelectedNote!.id!,
    ...
    async handleCancelShare() {
      ...
      if (this.selectedTab === "public") {
        await RootStore.Get(LuminaStore).shareNote.call({
          id: RootStore.Get(LuminaStore).curSelectedNote!.id!,
          isCancel: true,
        });
      }
    ...
    async loadTeamMembers() {
      ...
      try {
        ...
        const sharedUsers = await RootStore.Get(LuminaStore).getInternalSharedUsers.call(
          RootStore.Get(LuminaStore).curSelectedNote!.id!
        );
    ...
    <Card shadow="none" className="flex flex-col gap-2 p-2">
      <div className="w-full mb-2">
        <div className="flex p-1 gap-2">
          ...
          >
            <i className="ri-global-line" style={{ fontSize: '20px' }} />
            {t("public-share")}
          </Button>
          <Button
          ...
          >
            <i className="ri-lock-unlock-line" style={{ fontSize: '20px' }} />
            {t("internal-share")}
          </Button>
        </div>
      </div>
            ...
            <div className="flex items-center gap-2 ">
              ...
              <AnimatePresence mode="wait">
                {store.settings.expiryDate && (
                  ...
                  >
                    <i className="ri-time-line text-[#F68C06]" style={{ fontSize: '20px' }} />
              ...
              <Dropdown>
                <DropdownTrigger>
                  <Button
                    ...
                    startContent={<i className="ri-calendar-line text-default-500" style={{ fontSize: '20px' }} />}
                  >
              ...
              {store.expiryType === "custom" && (
                ...
                >
                  <PopoverTrigger>
                    <Button
                      size="lg"
                      startContent={<i className="ri-calendar-check-line text-default-500" style={{ fontSize: '20px' }} />}
                    >
          ...
          <div className="flex flex-col gap-2">
            {store.isLoadingUsers ? (
              <div className="flex justify-center p-4">
                <i className="ri-loader-4-line text-primary animate-spin" style={{ fontSize: '24px' }} />
              </div>
            ) : (
