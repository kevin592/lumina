...
import { UserAvatar } from '../LuminaCard/commentButton';
import { useEffect } from 'react';
import { LoadingAndEmpty } from '../Common/LoadingAndEmpty';
import { ScrollArea } from '../Common/ScrollArea';

export const LuminaSiteUser = observer(
  ({
    item,
    showFollow = true,
    onConfirm,
      ...
      <div className="flex items-center gap-1 mt-2 w-full">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1">
            {tags?.map((tag) => (
              <div className="Lumina-tag !text-xs mt-2">{tag}</div>
            ))}
          </div>
        </div>
        <Button
...
export const LuminaFollowDialog = observer(({ onConfirm }: { onConfirm: () => void }) => {
  const { t } = useTranslation();
  ...
  return (
    <div>
      <Input
        ...
        label={t('site-url')}
        placeholder={'https://www.Lumina.com'}
        endContent={
          <div className="flex items-center gap-2">
            <Button
              ...
              color="primary"
              radius="full"
              startContent={<Icon icon="ri:user-add-line" className="w-4 h-4" />}
            >
              {t('follow')}
            </Button>
            <Button
              ...
              size="sm"
              radius="full"
              startContent={<Icon icon="ri:refresh-line" className="w-4 h-4" />}
            ></Button>
          </div>
        }
      />
...
export const LuminaFollowingDialog = observer(({ data, onConfirm, isFollowing = false }: { data: follows[]; onConfirm: () => void; isFollowing: boolean }) => {
  const { t } = useTranslation();
  return (
