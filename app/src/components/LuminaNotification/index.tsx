...
export const LuminaNotification = observer(() => {
  ...
  if (store.unreadCount.value === 0 || Lumina.config.value?.isHiddenNotification) {
    return null
  }
        ...
        <div className="flex items-center justify-between p-4  w-full">
          ...
          {store?.unreadCount?.value > 0 && (
            <Button
              ...
              variant="light"
              size="sm"
              startContent={<i className="ri-check-double-line text-lg"></i>}
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="rounded-full bg-default-100 p-2">
                    <i
                      className={
                        notification.type === NotificationType.SYSTEM ? 'ri-notification-badge-line' :
                          notification.type === NotificationType.COMMENT ? 'ri-chat-3-line' :
                            notification.type === NotificationType.FOLLOW ? 'ri-user-star-line' :
                              'ri-user-add-line'
                      }
                      style={{ fontSize: '20px' }}
                    ></i>
