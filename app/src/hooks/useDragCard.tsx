...
interface DraggableLuminaCardProps {
  LuminaItem: any;
...
export const DraggableLuminaCard = ({ LuminaItem, showInsertLine, insertPosition, isDragForbidden }: DraggableLuminaCardProps) => {
  const { t } = useTranslation()
  
  const { setNodeRef: setDroppableRef, isOver } = useDroppable({
    id: `drop-${LuminaItem.id}`,
  });
        ...
        {isOver && isDragForbidden && !isDragging && (
          <div className="absolute inset-0 flex items-center justify-center z-50 bg-red-500/10 rounded-lg">
            <div className="bg-red-500 text-white rounded-full p-3">
              <i className="ri-prohibit-line text-3xl" />
            </div>
          </div>
        )}
        {isDragging ? (
          ...
          // The drag will be prevented by checking LuminaItem.isExpand in the card's event handlers
          <div
            ref={setDraggableRef}
            style={dragStyle}
            {...attributes}
          ...
          >
            <LuminaCard LuminaItem={LuminaItem} />
          </div>
        )}
      </div>
