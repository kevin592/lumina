...
container_name="lumina-website"
image_name="luminaspace/lumina:latest"
backup_dir="lumina-backup-$(date +"%Y%m%d_%H%M%S")"
docker_volume=""
...
if [ "$(docker ps -q -f name=$container_name)" ]; then
  docker cp "${container_name}:/app/.lumina" "$backup_dir"
...
# Step 2: Remove existing luminaspace/lumina Docker image
echo -e "${YELLOW}1. 🗑 Removing existing luminaspace/lumina Docker image...${NC}"
docker rmi "$image_name" -f

# Check if the container has a volume mounted
volume_path=$(docker inspect -f '{{ range .Mounts }}{{ if eq .Destination "/app/.lumina" }}{{ .Source }}{{ end }}{{ end }}' "$container_name")
if [ -z "$volume_path" ]; then
  ...
  docker_volume="-v $volume_path:/app/.lumina"
fi
...
# Step 3: Run the lumina-website container
echo -e "${YELLOW}2. ⏳ Starting the lumina-website container...${NC}"
