    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        if (row.type === 'COMMENT') {
          ...
          const luminaNote = await prisma.notes.findFirst({ where: { content: parentMemo.content } });
          if (!luminaNote) {
            yield { type: 'error', content: row?.content, error: new Error('parent lumina note not found'), progress: { current: i + 1, total } };
            continue;
          }

          let luminaComment = await prisma.comments.findFirst({ where: { content: row.content, noteId: luminaNote.id } });
          if (luminaComment) {
          ...
          luminaComment = await prisma.comments.findFirst({ where: { content: row.content, noteId: luminaNote.id } });
          if (luminaComment) {
            await prisma.comments.update({
              where: { id: luminaComment.id },
              data: {
            ...
            yield {
              type: 'success',
              content: luminaComment.content.slice(0, 30),
              progress: { current: i + 1, total }
            };
          }
        } else {
          const luminaNote = await prisma.notes.findFirst({ where: { content: row.content } });
          if (luminaNote && luminaNote.type !== -1) {
            yield { type: 'skip', content: row?.content, progress: { current: i + 1, total } };
            continue;
          }
